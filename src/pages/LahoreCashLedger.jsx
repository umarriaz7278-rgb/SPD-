...
const LahoreCashLedger = () => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      console.error('Error fetching Lahore cash ledger:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (type) => {
    ...
    try {
      ...
      if (type === 'Expense') {
        await autoTransferToLedger(formData.description, Number(formData.amount), formData.date, 'lahore_cash_ledger', 'Lahore Cash Ledger');
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const { error } = await supabase.from('lahore_cash_ledger').delete().eq('id', id);
      if (error) throw error;
  ...
  return (
    <div style={styles.root}>
      ...
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}><Wallet size={26} color="#f59e0b" /> Lahore Cash Ledger</h1>
          <div style={styles.subtitle}>Manage Lahore office income & expenses — Net balance updated automatically</div>
        </div>
      </div>
...
export default LahoreCashLedger;
