...
import { Truck, ArrowLeft, Loader2, DollarSign, Route, FileText, Plus, Receipt, AlertCircle } from 'lucide-react';
...
const VehicleDetail = () => {
  ...
  const [expForm, setExpForm] = useState({ date: new Date().toISOString().split('T')[0], description: '', amount: '', payment_type: 'Cash', supplier_id: '' });
  ...
  const handleAddExpense = async (e) => {
    ...
    try {
      ...
      setExpForm({ date: new Date().toISOString().split('T')[0], description: '', amount: '', payment_type: 'Cash', supplier_id: '' });
               ...
               <div className="table-wrapper" style={{ border: 'none' }}>
                 <table>
                   ...
                   <tbody>
                     ...
                     {trips.length === 0 && <tr><td colSpan={3} className="text-center text-muted py-8">No trips recorded yet.</td></tr>}
                   </tbody>
                 </table>
               </div>
            </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            {showAddExpense && (
              <div className="card bg-danger-light border-danger-light">
                ...
                <form onSubmit={handleAddExpense} className="grid grid-cols-12 gap-4 items-end">
                  ...
                  <div className="input-group col-span-1">
                    <button type="submit" className="btn btn-danger w-full" disabled={savingExpense} style={{ height: '42px', padding: 0 }}>
                      {savingExpense ? <Loader2 size={16} className="animate-spin"/> : 'Add'}
                    </button>
                  </div>
                </form>
              </div>
               ...
               <div className="table-wrapper" style={{ border: 'none' }}>
                 <table>
                   ...
                   <tbody>
                     ...
                     {expenses.length === 0 && <tr><td colSpan={5} className="text-center text-muted py-8">No expenses recorded yet.</td></tr>}
                   </tbody>
                 </table>
               </div>
            </div>
