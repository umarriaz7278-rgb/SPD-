...
const MainOwnerCashLedger = () => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      console.error('Error fetching Main Owner cash ledger:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (type) => {
    ...
    try {
      ...
      if (type === 'Expense') {
        await autoTransferToLedger(formData.description, Number(formData.amount), formData.date, 'main_owner_cash_ledger', 'Main Owner Cash Ledger');
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const { error } = await supabase.from('main_owner_cash_ledger').delete().eq('id', id);
      if (error) throw error;
  ...
  return (
    <div style={styles.root}>
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}><Wallet size={26} color="#f59e0b" /> Main Owner Cash Ledger</h1>
          <div style={styles.subtitle}>Manage main owner income & expenses — Net balance updated automatically</div>
        </div>
      </div>
...
export default MainOwnerCashLedger;
